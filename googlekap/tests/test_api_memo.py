...
def test_get_memo(client, memo_data):
    r = client.get("/api/memos/1", follow_redirects=True)
...
def test_post_memo(client, memo_data):
    r = client.post("/api/memos", data=memo_data)
...
def test_post_memo_with_img(client, memo_data):
    ...
    r = client.post("/api/memos", data=data)
...
def test_put_memo_with_img(client, memo_data):
    ...
    data = {"linked_image": (BytesIO(b"dummy"), "test.jpg")}
    r = client.put(f"/api/memos/{memo_id}", data=data)
    assert r.status_code == 200
    assert r.json.get("linked_image") is not None
...
def test_delete_memo_with_img(client, memo_data):
    ...
    memo_id = r.json["id"]
    r = client.delete(f"/api/memos/{memo_id}/image")
    assert r.status_code == 204

    r = client.get(f"/api/memos/{memo_id}", follow_redirects=True)
